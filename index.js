...
const client = new Client({
    ...
    presence: {
        activities: [{
            name: `BOT_PRESENCE_NAME`,
            type: "BOT_TYPE", // watching, playing, listing
        }],
        status: "BOT_STATUS" // online, dnd, idle, invisible
    }
});
  ...
  try
    {
        ...
        if (config.Servers)
        {
            // if isnt the bot ... REMOVE THIS LINE IF YOU WANT THIS FOR MULTIPLE BOTS
            if (oldPresence.user.id !== config.Bot) return;
            /* Bot went online. */
            if (newPresence.status == PresenceUpdateStatus.Online)
            {
                newPresence.guild.channels.cache.get(config.Channels).send({
                    ...
                    .setFooter({ text:`Uptime Alert`, iconURL: newPresence.user.avatarURL({ format: "png", size: 1024 }) })
                    .setTimestamp()]
                });
            }
            ...
            else if (newPresence.status == PresenceUpdateStatus.Offline || newPresence.status == PresenceUpdateStatus.Invisible)
            {
                newPresence.guild.channels.cache.get(config.Channels).send({
                    ...
                    .setColor(`RED`)
                    .setThumbnail(newPresence.user.avatarURL({ format: "png", size: 1024 }))
                    .setFooter({ text:`Downtime Alert`, iconURL: newPresence.user.avatarURL({ format: "png", size: 1024 }) })
                    .setTimestamp()]
                });
            }
        }
...
client.on("ready", async () => {
  try{
      client.logger(`${client.user.tag} is online!`.bold.brightGreen);
